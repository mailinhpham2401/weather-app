...
export default {
  ...
  methods: {
    ...
    fetchWeatherForecast() {
      axios
        .get(
          `${this.url_base}forecast?APPID=${this.api_key}&q=${this.query}&units=metric`
        )
        .then((response) => {
    ...
    ForeCastOfCurrentCity() {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position) => {
          axios
            .get(
              `${this.url_base}forecast?APPID=${this.api_key}&lat=${position.coords.latitude}&lon=${position.coords.longitude}&units=metric`
            )
            .then((response) => {
              this.weathers = response.data.list.filter((weather) => {
                return weather["dt_txt"].includes(this.PREFERRED_TIME);
